...
SLACK_API = os.environ["SLACK_WEBHOOK"]
...
def get_movie():
    ...
    try:
        ...
        r = requests.get(url = MOVIEDB_API + f"?api_key={MOVIEDB_API_KEY}&include_adult=false&page=1&query={query}", headers = headers, data = payload, timeout = 5.0)
        response = r.json()
        global slack_title
        ...
        print(f'Failed to connect to the MovieDB site ', e)
        raise
...
def post_to_slack(results, counter):
    ...
    try:
        ...
        print(f'Failed to connect to the slack webhook. Ensure your webhook is still valid. ', e)
        raise
    except HTTPError as e:
        print(e)
        raise
